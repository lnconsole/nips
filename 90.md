...
## image background removal: `kind:65007`

| param     | req? | description                                           |
|-----------|------|-------------------------------------------------------|

## image overlay: `kind:65008`

Overlay an image on top of another. Two images MUST be provided. Use the `background` marker to indicate which one is the base image. If marker is absent, the first image in the array is assumed to be the base image

```
[ i , <image url>, url, background ]  // base image
[ i , <image url>, url ]              // image to be overlaid on top of the base image
```
